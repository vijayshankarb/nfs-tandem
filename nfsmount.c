...
buffer[0] ='\0';
flags = ((MS_MGC_VAL & ~MS_RDONLY) & ~MS_NOSUID);
nfsmount(sys,mp, &flags, &eo, &buf,1);
return mount((const char*)sys,(const char*)mp,"nfs", flags, buf);
}
